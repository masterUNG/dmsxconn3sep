...
// class AddInformationUser extends StatelessWidget {
//   const AddInformationUser({super.key});
...
class _AddInformationUserState extends State<AddInformationUser> {
  ...
  Future<Null> readCurrentInfo() async {
    ...
    setState(() {
      userModel = UserModel(
          ...
          user_branch: preferences.getString('user_branch') ?? '',
  ...
  Widget showContent() => SingleChildScrollView(
        child: Column(
          ...
          children: [
            SizedBox(height: 20),
            // showImage(),
            elementShowText(head: 'ชื่อ', value: userModel!.staffname ),
            SizedBox(height: 8),
            elementShowText(head: 'สาขา', value: userModel!.user_branch ),
            SizedBox(height: 8),
            elementShowText(head: 'ตำแหน่ง', value: map[userModel!.user_type] ?? '' ),
            // addressFrom(),
            // phoneFrom(),
            // nameBankFrom(),
            // numberBankFrom(),
  ...
  Widget elementShowText({required String head, required String value}) => Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          SizedBox(
            width: 300,
